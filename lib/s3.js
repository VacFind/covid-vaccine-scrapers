...
async function uploadFile(bucketPath, fileName) {
    ...
    if (process.env.DEVELOPMENT) {
        bucketName = process.env.AWSS3BUCKETNAMETEST;
    } else {
...
async function saveErrorLog(error) {
    const errorFileName = `error-${getTimestampForFile()}.log`;
    file.write(errorFileName, error);
    file.write(errorFileName, error);
    const s3Dir = "/";
    const upload = await uploadFile(s3Dir, errorFileName);
    return upload;
}
