...
async function uploadFile(bucketPath, localPath, remotePath) {
    if (!bucketPath) {
    ...
    const fileContents = file.read(localFileName);
    const params = {
 ...
 * @param {string} path - can have trailing slash
 * @param {int} maxKeys - ideally greater than zero to get results
 * @returns {object} found keys
 */
